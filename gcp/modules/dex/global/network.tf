...
locals {
  hostname = trimsuffix("oauth2.${var.dns_domain_name}", ".")
}
...
resource "google_certificate_manager_dns_authorization" "dex_auth" {
...
resource "google_dns_record_set" "CNAME_auth_dex" {
  count = var.single_region ? 0 : 1

  project = var.project_id

  name = google_certificate_manager_dns_authorization.dex_auth[count.index].dns_resource_record[0].name
  type = google_certificate_manager_dns_authorization.dex_auth[count.index].dns_resource_record[0].type
  ttl  = 60

  managed_zone = var.dns_zone_name
  rrdatas      = [google_certificate_manager_dns_authorization.dex_auth[count.index].dns_resource_record[0].data]
}

resource "google_compute_global_address" "gce_lb_ipv4" {
  name         = var.lb_address_name == "" ? format("oauth2-%s-gce-ext-lb", var.cluster_name) : var.lb_address_name
...
resource "google_certificate_manager_certificate" "ssl_certificate" {
  ...
  managed {
    ...
    dns_authorizations = [
      google_certificate_manager_dns_authorization.dex_auth[count.index].id
    ]
  }
}

resource "google_certificate_manager_certificate_map" "dex_certificate_map" {
  count = var.single_region ? 0 : 1

  name = "dex-cert-map"
}

resource "google_certificate_manager_certificate_map_entry" "dex_certificate_map_entry" {
  count = var.single_region ? 0 : 1

  name         = "dex-cert-map-entry"
  map          = google_certificate_manager_certificate_map.dex_certificate_map[count.index].name
...
resource "google_compute_target_https_proxy" "lb_proxy" {
  ...
  name    = "dex-https-proxy"
  project = var.project_id

  url_map = google_compute_url_map.url_map[count.index].id

  ssl_policy      = google_compute_ssl_policy.ssl_policy[count.index].id
  certificate_map = "//certificatemanager.googleapis.com/${google_certificate_manager_certificate_map.dex_certificate_map[count.index].id}"
}

resource "google_compute_global_forwarding_rule" "https_forwarding_rule" {
  count = var.single_region ? 0 : 1

  name    = "dex-https-forwarding-rule"
  project = var.project_id
