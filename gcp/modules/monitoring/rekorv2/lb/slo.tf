    ...
    gcs-read-availability = {
      ...
      slos = {
        api-v2-read = {
          ...
          label_filter   = "resource.labels.forwarding_rule_name=monitoring.regex.full_match(\".*-rekor-https-forwarding-rule\") resource.labels.backend_target_type=\"BACKEND_BUCKET\""
          goal           = 0.995
        },
      }
    }
