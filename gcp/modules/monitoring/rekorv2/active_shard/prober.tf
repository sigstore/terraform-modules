...
resource "google_monitoring_alert_policy" "prober_rekorv2_endpoint_latency" {
  ...
  conditions {
    ...
    display_name = "API Prober: Rekor v2 API Endpoint Latency > 750 ms"
  }

  display_name = "API Prober: Rekor v2 API Endpoint Latency > 750 ms for 5 minutes"

  documentation {
    content   = "At least one supported Rekor v2 API Endpoint has had latency > 750 ms for 5 minutes."
