...
locals {
  cluster_namespace      = format("%s-%s", var.shard_name, var.cluster_namespace_suffix)
...
resource "google_compute_global_address" "gce_lb_ipv4" {
  name         = format("%s-%s-%s-gce-ext-lb", var.shard_name, var.dns_subdomain_name, var.cluster_name)
